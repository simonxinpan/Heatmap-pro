...
class DataProcessor {
    ...
    processStockData(stocks) {
        return stocks.map(stock => ({
            ...
            marketCap: parseInt(stock.market_cap || stock.marketCap) || 0,
            industry: stock.industry || '其他',
            sector: stock.sector || '其他',
            tags: stock.tags || [],
            // 计算热力图所需的额外字段
            size: this.calculateSize(stock),
            color: this.calculateColor(stock.change_percent || stock.changePercent),
    ...
    groupByIndustry(stocks) {
        ...
        stocks.forEach(stock => {
            ...
            industries[industry].totalMarketCap += stock.marketCap;
        ...
        Object.values(industries).forEach(industry => {
            if (industry.stocks.length > 0) {
                industry.avgChange = industry.stocks.reduce((sum, stock) => 
                    sum + stock.changePercent, 0) / industry.stocks.length;
            }
        });
            ...
            try {
                ...
                if (processedTag.stocks.length > 0) {
                    processedTag.avgChange = processedTag.stocks.reduce((sum, stock) => 
                        sum + stock.changePercent, 0) / processedTag.stocks.length;
                    processedTag.totalMarketCap = processedTag.stocks.reduce((sum, stock) => 
                        sum + stock.marketCap, 0);
        ...
        return trends.map(trend => ({
            ...
            if (trend.stocks.length > 0) {
                trend.avgChange = trend.stocks.reduce((sum, stock) => 
                    sum + stock.changePercent, 0) / trend.stocks.length;
    ...
    // 计算股票大小（用于热力图）
    calculateSize(stock) {
        const marketCap = parseInt(stock.market_cap || stock.marketCap) || 0;
    ...
    filterData(data, filters = {}) {
        ...
        if (filters.minChange !== undefined) {
            filtered = filtered.filter(item => 
                (item.changePercent || item.avgChange || 0) >= filters.minChange
            );
        }
        
        if (filters.maxChange !== undefined) {
            filtered = filtered.filter(item => 
                (item.changePercent || item.avgChange || 0) <= filters.maxChange
            );
        }
        ...
        if (filters.minMarketCap !== undefined) {
            filtered = filtered.filter(item => 
                (item.marketCap || item.totalMarketCap || 0) >= filters.minMarketCap
            );
        }
    ...
    // 数据排序
    sortData(data, sortBy = 'changePercent', order = 'desc') {
        return [...data].sort((a, b) => {
            ...
            // 处理特殊字段
            if (sortBy === 'avgChange') {
                aValue = a.avgChange || a.changePercent || 0;
                bValue = b.avgChange || b.changePercent || 0;
            }
