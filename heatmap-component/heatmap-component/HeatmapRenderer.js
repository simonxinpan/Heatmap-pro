...
class HeatmapRenderer {
    ...
    addStyles() {
        if (document.getElementById('heatmap-renderer-styles')) {
            return;
        }
        
        const style = document.createElement('style');
        style.id = 'heatmap-renderer-styles';
        style.textContent = `
            .heatmap-cell {
                stroke: #fff;
                stroke-width: 1;
    ...
    render(data, metric = 'changePercent') {
        this.data = data;
        this.metric = metric;
    ...
    createScales(data, metric) {
        ...
        // 大小比例尺（基于市值或成交量）
        const sizeValues = data.map(d => d.marketCap || d.market_cap || d.volume || d.size || 1);
    ...
    renderCells(layout, metric) {
        layout.forEach((item, index) => {
            ...
            const sizeScale = this.scales.size(item.marketCap || item.market_cap || item.volume || item.size || 1);
    ...
    showTooltip(event, data) {
        ...
        this.tooltip.innerHTML = `
            ...
            ${data.marketCap || data.market_cap ? `<div>市值: ${this.formatMarketCap(data.marketCap || data.market_cap)}</div>` : ''}
    ...
    getValue(item, metric) {
        switch (metric) {
            case 'changePercent':
            case 'change_percent':
                return parseFloat(item.changePercent || item.change_percent || item.avgChange || 0);
            case 'volume':
                return parseInt(item.volume || item.totalVolume || 0);
            case 'marketCap':
            case 'market_cap':
                return parseInt(item.marketCap || item.market_cap || item.totalMarketCap || 0);
            case 'price':
